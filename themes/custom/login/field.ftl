<#-- Copied from keycloak.v2/login/field.ftl (26.0.8). Only change: `input` and
     `password` macros gain a `placeholder` param so login.ftl can set field hints. -->
...
<#macro input name label value="" required=false autocomplete="off" fieldName=name autofocus=false placeholder="">
...
<#macro password name label value="" required=false forgotPassword=false fieldName=name autocomplete="off" autofocus=false placeholder="">
  <#assign error=kcSanitize(messagesPerField.get(fieldName))?no_esc>
  <@group name=name label=label error=error required=required>
